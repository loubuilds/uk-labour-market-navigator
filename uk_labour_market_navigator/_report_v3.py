...
def render(plan, evidence, draft):
    if evidence["schema_version"] != 2:
        from ._report_v2 import render as previous_render

        return previous_render(plan, evidence, draft)
    ...
    def citation_label(fact_id):
        ...
        labels = {
            "new_adverts": "new adverts, April to June 2026",
        ...
        place = next(row["place"] for row in [*rows, *demand["rows"]] if row["id"] == place_id)
        return place + ": " + labels[measure]

    role = evidence["scope"]["occupation"]["label"]
    names = ", ".join(p["label"] for p in evidence["scope"]["places"])
    scope = f"{role} in {names}. Whole occupation, all seniority levels. "
    if rows:
        scope += "Census Day: 21 March 2021. Usual residents aged 16+ in employment, on districts tabulated in the May 2023 release. "
    scope += "Employer advertising: April to June 2026, on April 2023 districts. These are separately scoped measures, not a single current labour pool or a commuting catchment."
    html = [
        '<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1">',
        f"<title>{escape(draft['title'])}</title><style>{CSS}</style></head><body><main><header>",
        '<div class="brand">UK Labour Market Navigator / Research brief</div><p class="small">Coverage varies: Census workforce context for England and Wales; new-advert evidence for supported UK districts.</p>',
    ...
    if rows:
        ...
        html.append(
            ...
            "Read this historical structure alongside the separate new-advert evidence below. The two measures do not establish candidate availability or how easy hiring will be.</p>"
        )
        md.append(
            "Hiring picture: incomplete. This is historical workforce context; read it alongside the separate new-advert evidence. Neither measure establishes candidate availability or ease of hiring."
        )
        if len(rows) == 1:
            row = rows[0]
    ...
    elif plan["include_workforce"]:
        missing = [g["place"] + ": " + g["reason"] for g in census["gaps"]]
        html.append(
            '<section class="panel open"><h2>Resident workforce context unavailable</h2><p>'
            + escape("; ".join(missing))
            + "</p><p>The employer-advertising section is checked independently. No missing workforce count has been treated as zero.</p></section>"
        )
        md.extend(["## Resident workforce context unavailable", *missing])
    html.append(
        '<section class="panel"><div class="label">Checked employer demand signal</div><h2>New adverts for this occupation</h2>'
    )
    md.extend(
        [
            "## New adverts for this occupation",
            "April to June 2026. April 2023 district boundaries. Occupation: " + demand["scope"]["occupation"]["label"],
        ]
    )
    if demand["facts"]:
        quality = "April to June 2026. June figures are partly estimated by ONS because of source problems; uncertainty has increased since November 2025. These are new adverts during the quarter, not live vacancies or distinct employers."
        html.append('<p class="scope">' + escape(quality) + "</p>")
        html.append(
            '<div class="cards">'
            + "".join(
                '<div class="card"><span class="value">'
                + f"{int(row['new_adverts']):,}"
                + "</span><strong>New online adverts</strong><p>"
                + escape(demand["scope"]["occupation"]["label"] + " in " + row["place"])
                + "</p></div>"
        ...
        html.append(
            '<p class="small">Source: <a href="'
            + escape(demand["source"]["source_page"], quote=True)
            + '">ONS, Labour demand volumes by SOC2020, released 21 August 2026, Table 5</a>. Advert locations use April 2023 districts. This saved edition is not a live refresh.</p>'
        )
        md.extend([quality, *["- " + f["display"] for f in demand["facts"]]])
        if len(demand["rows"]) > 1:
            html.append(
                "<p>Counts describe the volume of new advertising in each selected district. A larger count does not by itself mean stronger competition for each candidate.</p>"
            )
        html.append(
            "<p><strong>What this adds:</strong> a recent, dated view of advertising activity for the occupation. Keep it alongside workforce structure; comparing adverts with historical employed residents would not produce a defensible competition or shortage score.</p>"
        )
    else:
        html.append("<ul>" + "".join("<li>" + escape(g) + "</li>" for g in demand["gaps"]) + "</ul>")
        md.extend(demand["gaps"])
    html.append("</section>")
    md.extend(
        [
            "## What to take into your discussion",
    ...
    gaps = list(
        dict.fromkeys(
            [
                *evidence["gaps"],
                "Available candidates, detailed skills, seniority and employer-specific hiring causes are not measured by these occupation-level sources.",
                "Pay and wider local indicators are not yet included in this build. New-advert counts do not identify distinct competing employers or currently open vacancies.",
            ]
        )
    )
    html.append(
    ...
    routes = [
        (
            "What are employers advertising?",
            "Optional Adzuna can show recent advert examples, advertiser names, description excerpts and stated pay bounds where supplied. These remain separate from the checked official report.",
            "Available with your own permitted account. Say connect Adzuna when useful.",
        ),
        (
            "How much new advertising is there?",
            ...
            "Available now for supported districts; see this report's actual coverage above.",
        ),
        (
    ...
    html.append(
        ...
        + '</ul><p class="small">You can compare another district, explore employer advertising on its own, change the occupation, or show the sources. Pay and radius research remain planned.</p></section>'
    )
    md.extend(["## A useful next conversation", *["- " + q for q in draft["next_questions"]]])
    url = demand["source"]["source_page"]
    html.append(
        '<section class="panel"><h2>Where the numbers come from</h2><p><a href="'
        + escape(url, quote=True)
        + '">ONS new-advert tables, released August 2026</a></p>'
    )
    if rows:
        html.append(
            '<p><a href="'
            + escape(census["source"]["source_page"], quote=True)
            + '">ONS Census occupation tables, released May 2023</a></p>'
        )
        md.append("Census source: " + census["source"]["source_page"])
    html.append(
    ...
    for fact in facts.values():
        ...
        meaning = {
            "new_adverts": "Online adverts first appearing during April to June 2026, assigned to this occupation and district by ONS. One advert may cover several jobs; an employer can place several adverts. This does not count distinct competitors or people available to hire.",
        ...
        period_label = "Census Day: 21 March 2021. "
        if measure == "new_adverts":
            period_label = "April to June 2026. "
            method = "Read from ONS Table 5, the published new-advert cell for this occupation and April 2023 district. June is partly imputed; increased source uncertainty remains attached to the figure. No growth rate is calculated."
            trace = "; ".join(f"{c['geography_label']}: Table 5, cell AR{c['row']}" for c in fact["source_cells"])
        html.append(
            '<details id="fact-'
            ...
            + escape(meaning)
            + "</p>"
            + "<p><strong>When and who</strong><br>"
            + escape(period_label + population)
            + ".</p>"
        ...
        md.extend(
            [
                "### " + fact["display"],
                meaning,
                "When and who: " + period_label + population + ".",
    ...
    methodology = [
        "The workforce section, where available, describes where employed people lived at Census 2021, across all seniority levels in the named occupation. They do not describe today's available candidates.",
    ...
    methodology.extend(demand["caveats"])
    technical = {
        "scope": {"workforce": census["scope"] if plan["include_workforce"] else None, "demand": demand["scope"]},
        "source": {"workforce": census["source"], "demand": demand["source"]},
