...
from ._report_v3 import CSS, chart
...
def render(plan, evidence, draft):
    ...
    if p.get("status") != "not_requested":
        ...
        if p["facts"]:
            ...
            th, tm = table(["Earnings measure", "UK benchmark", "Sampling uncertainty (CV)", "Source precision"], rows)
            h += th
            m.append(tm)
            scope = p["scope"]
            note = (
                ...
                + ". All sexes; workplace basis; 2025 provisional, with the December correction retained. Percentiles describe the pay distribution, not a recommended offer range. CV stands for coefficient of variation: the estimate's standard error as a percentage of the estimate. Lower CV means less sampling uncertainty. It does not describe differences between individual salaries or guarantee a particular salary."
            )
