...
from ._report_v3 import CSS as BASE_CSS
...
CSS = (
    BASE_CSS
    + """
:root{--ink:#173448;--teal:#07545b}body{background:#faf9f6}
.brief-nav{display:flex;flex-wrap:wrap;gap:10px;margin:10px 0 26px}.brief-nav a{padding:6px 12px;border:1px solid var(--line);border-radius:20px;background:white;font-size:.88rem;text-decoration:none}
.insights{display:grid;grid-template-columns:repeat(auto-fit,minmax(290px,1fr));gap:18px}.insight{background:#f5f8f6;border-top:3px solid #0c7776;border-radius:6px;padding:18px}.insight h3{margin:0 0 10px;font-size:1.12rem}.insight p{margin:0 0 10px}.insight details{margin:8px 0 0}.insight summary{padding:4px 0;font-size:.8rem}
.note{background:#fff7ef;border-left:3px solid #f87916;padding:12px 16px;font-size:.9rem}.scope-details{margin:0;border:0}.steps{padding-left:24px}.steps li{padding:5px 0 12px 12px}.steps li::marker{font-weight:750;color:var(--teal)}
@media(max-width:380px){.insights{grid-template-columns:1fr}}@media print{.brief-nav{display:none}.insights{display:block}.insight{break-inside:avoid;margin-bottom:12px}}
"""
)
...
def render(plan, evidence, draft):
    if evidence["schema_version"] not in (3, 4, 5):
    ...
    html.append(
        '<nav class="brief-nav" aria-label="Brief sections"><a href="#what-this-helps-you-assess">The decision</a><a href="#official-earnings-uk-benchmark">Pay evidence</a><a href="#next-steps">Next steps</a><a href="#where-the-numbers-come-from">Sources</a></nav>'
    )

    def section(title, content, markdown):
        anchor = re.sub(r"[^a-z0-9]+", "-", title.lower()).strip("-")
        html.append(
            '<section class="panel" id="' + anchor + '"><h2>' + escape(title) + "</h2>" + content + "</section>"
        )
    ...
    context_labels = {
        f["id"]: f["label"]
        for family in ("context", "economic", "hours")
        for f in evidence.get(family, {}).get("facts", [])
    }
    ...
    for s in draft["sections"]:
        h += (
            '<article class="insight"><h3>'
            + escape(s["heading"])
            + "</h3><p>"
            + escape(s["text"])
            + '</p><details><summary>Supporting figures</summary><p class="cite">'
        )
        labels = []
        for i in s["fact_ids"]:
    ...
    section("What this helps you assess", h + "</div>", "\n\n".join(m))
    ...
    section(
        "Your question and the evidence scope",
        '<details class="scope-details"><summary>View the original question and accepted scope</summary>'
        + bullets(question)
        + "</details>",
    ...
    if wider is not None:
        ...
        if wider["facts"]:
            ...
            h += (
                '<p class="small">'
                + escape(notes[0])
                + "</p><details><summary>How to read these comparisons</summary>"
                + bullets(notes[1:])
                + "</details>"
            )
            m.extend(notes)
    ...
    if p.get("status") != "not_requested":
        ...
        if p["facts"]:
            intro = (
                ...
                + " earnings for "
                + p["scope"]["source_occupation_label"].rstrip(".")
                + "."
            )
            if p["scope"]["measure"] == "annual_gross":
                intro += (
                    " Tax year ending 5 April 2025. Gross earnings include overtime and bonuses, before deductions; benefits in kind are excluded. This is a historical earnings benchmark, not a current Sheffield basic-salary offer."
                    if names == "Sheffield"
                    else " Tax year ending 5 April 2025. Gross earnings include overtime and bonuses, before deductions; benefits in kind are excluded. This is a historical earnings benchmark, not a current local basic-salary offer."
                )
            else:
                intro += " Pay period including 30 April 2025. Hourly earnings are before deductions and exclude overtime; this is not a basic-pay-only measure; benefits in kind are excluded. This is a UK benchmark, not a local offer."
            ...
            h += (
                "<details><summary>Period, eligibility and CV explained</summary><p>"
                + escape(note)
                + "</p></details>"
                + source_link(p["source"])
            )
            m.append(note)
        ...
        if hours and hours["status"] != "declined":
            h += "<h3>What does the paid working week look like?</h3>"
            m.append("### What does the paid working week look like?")
            if hours["facts"]:
                scope = hours["scope"]
                intro = (
                    "Separate UK weekly paid-hours benchmarks · "
                    + scope["source_occupation_label"]
                    + " · "
                    + scope["working_pattern"].replace("_", " ")
                    + " · April 2025."
                )
                h += '<p class="small">' + escape(intro) + "</p>"
                m.append(intro)
                th, tm = table(
                    ["Paid hours a week", "Mean (average)"],
                    [[r["label"], r["mean"]] for r in hours["rows"]],
                )
                h += th
                m.append(tm)
                note = "Pay and hours are separate benchmarks: these are not necessarily the hours worked by people earning the median annual pay. These averages include jobs with no paid overtime; unpaid work is excluded."
                h += '<p class="note">' + escape(note) + "</p>"
                m.append(note)
                h += (
                    "<details><summary>Hours definitions and precision</summary>"
                    + bullets(hours["caveats"])
                    + "<p>"
                    + escape(
                        hours["population"]
                        + ". "
                        + hours["period"]
                        + ". Each estimate has its own CV in the source ledger; medians are not additive."
                    )
                    + "</p></details>"
                )
            h += bullets(hours["gaps"])
            m.extend(hours["gaps"])
    ...
    if economic is not None:
        ...
        if economic["facts"]:
            ...
            h += (
                '<p class="note">Average earnings growth is not an individual pay award. These rates use different periods and cannot be used to update the occupation salary or calculate a recommended increase.</p><details><summary>What these measures include</summary>'
                + bullets(notes)
                + "</details>"
            )
            m.extend(notes)
    ...
    section(
        "Next steps",
        '<ol class="steps">' + "".join("<li>" + escape(q) + "</li>" for q in draft["next_questions"]) + "</ol>",
        "\n".join("- " + md_text(x) for x in draft["next_questions"]),
    )
    h = '<details class="scope-details"><summary>Browse the checked figures and their sources</summary>'
    m = []
    for f in evidence["facts"]:
        ...
        h += (
            ...
            + '"><summary>'
            + escape(reference_label(f["id"]))
            + "</summary><p>"
            + escape(display)
            + "</p><p><strong>When and who</strong><br>"
            + escape(meaning)
    ...
    section(
        ...
        h + "</details>",
        "<details><summary>Browse the checked figures and their sources</summary>\n\n"
        + "\n\n".join(m)
        + "\n\n</details>",
    )
    if evidence["pay"].get("status") == "not_requested":
        html = [part.replace('<a href="#official-earnings-uk-benchmark">Pay evidence</a>', "") for part in html]
